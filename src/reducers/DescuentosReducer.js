      ...
      export default (state = INITIAL_STATE, action) => {
        switch (action.type) {
          ...
          case "CLEAR_DESCUENTOS":
            let descuentosss = [...state.descuentos];
            let nuevoIndex = descuentosss.findIndex(descuento => descuento.idDescuento === "nuevo");
            if(nuevoIndex !== -1) descuentosss.splice(nuevoIndex, 1);
            return { ...state, descuento: undefined, descuentos: descuentosss };
          default:
            return { ...state };
        }
      };
